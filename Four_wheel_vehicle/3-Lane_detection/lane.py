...
# Modified/upgrade : Yu Zheng, Resilient & Autonomous systems lab, Florida state University
# (1) refine a more reliable "trapezoid-shaped region of interest" setting, 
#     which adjusts the region of interest based on the size of image automatically
# (2) add color-based lane detection when the image light is too dark
# (3) add lane reconstruction mechanism: when lose the track of one of the lane, construct the lost lane
 
# filename = 'original_lane_detection_3.png'
...
class Lane:
    ...
    def __init__(self, orig_frame):
        ...
        mid_width =  0.65 # percentage of mid trapezoidal height
        height_pct = 0.58 #.58 # percentage of trapezoidal height
        bottom_trim= 0.9 # percentage from top to bottom avoiding the hood of the car

        self.roi_points = np.float32([
        (width*(0.5-mid_width/2), height*height_pct), # Top-left corner
        (width*(0.5-bot_width/2), height*bottom_trim), # Bottom-left corner            
        (width*(0.5+bot_width/2), height*bottom_trim), # Bottom-right corner
        (width*(0.5+mid_width/2), height*height_pct)  # Top-right corner
        ])
        ...
        self.desired_roi_points = np.float32([[self.padding, 0], # Top-left corner
        ...
        self.lane_width = 50.0  # initial lane width in image frame
        self.left_fit = None  # np.array([2.54219906e-04, -3.99957060e-01,  1.37618857e+02])
        self.right_fit = None # np.array([-4.33800744e-04,  4.12864241e-01 , 1.66722430e+02])
        ...
        # Yu: (This parameter is decided by camera, adjust it during experiment)
        self.YM_PER_PIX = 10.0 / 720 # meters per pixel in y dimension
        self.XM_PER_PIX = 4 / 384 # meters per pixel in x dimension
            
        # Radii of curvature and offset
        self.left_curvem = None
        self.right_curvem = None
        self.center_offset = None
        self.center_curvem = None
    ...
    def calculate_car_position(self, print_to_terminal=False):
        ...
        if print_to_terminal == True:
            print('center offset:' + str(center_offset) + ' m')
    ...
    def calculate_curvature(self, print_to_terminal=False):
        ...
        if print_to_terminal == True:
            print('left lane curvature:', left_curvem, 'm,', 'right lane curvature:', right_curvem, 'm')
                
        self.left_curvem = left_curvem
        self.right_curvem = right_curvem
        self.center_curvem = (self.left_curvem+self.right_curvem)/2
    ...
    def display_curvature_offset(self, frame=None, plot=False):
        ...
        cv2.putText(image_copy,'Curve Radius: '+str((self.left_curvem+self.right_curvem)/2)[:7]+' m', (int((5/600)*self.width), int((20/338)*self.height)), cv2.FONT_HERSHEY_SIMPLEX, (float((0.5/600)*self.width)),(255,255,255),2,cv2.LINE_AA)
        cv2.putText(image_copy,'Center Offset: '+str(self.center_offset)[:7]+' m', (int((5/600)*self.width), int((40/338)*self.height)), cv2.FONT_HERSHEY_SIMPLEX, (float((0.5/600)*self.width)),(255,255,255),2,cv2.LINE_AA)
    ...
    def get_lane_line(self):
        self.leftx = []
        self.rightx = []
        self.lefty = []
        self.righty = []
        for iter in range(np.int(self.height/10)):
            slice_frame = self.warped_frame[:,iter*10:(iter+1)*10]
            self.histogram = self.calculate_histogram(frame=slice_frame)
            leftx, rightx = self.histogram_peak()
            self.leftx.append(leftx)
            self.rightx.append(rightx)
            if leftx != None:
                self.lefty.append(5+iter*10)
            if rightx != None:
                self.righty.append(5+iter*10)

        # if one lane is not detected
        if np.size(self.rightx) == 0:
            self.rightx = self.leftx + self.width/2
            self.righty = self.lefty.copy()

        if np.size(self.leftx) == 0:
            self.leftx = self.rightx - self.width/2
            self.lefty = self.righty.copy()
    ...
    def get_lane_line_previous_window(self, left_fit, right_fit, plot=False):
        ...
        if np.size(leftx) == 0:
            lefty = righty
            leftx = rightx - self.width/2
        
        if np.size(rightx) == 0:
            righty = lefty
            rightx = leftx + self.width/2

        '''
        # Yu: get average lane width
        if not np.size(leftx) == 0 and not np.size(rightx) == 0:
            k = np.minimum(np.size(leftx),np.size(rightx))
            self.lane_width = np.average(abs(rightx[0:k] - leftx[0:k]))
        
        # Yu: reconstruct the loss lane
        if np.size(leftx) == 0:
            leftx = rightx - self.lane_width
            lefty = righty
        if np.size(rightx) == 0:
            rightx = leftx + self.lane_width
            righty = lefty  
        '''     
        
        # Fit a second order polynomial curve to each lane line
        if not np.size(leftx) == 0 or not np.size(leftx) == 0:
            self.leftx = leftx
            self.lefty = lefty
            self.rightx = rightx
            self.righty = righty 
    ...
    def blue_mask(self,frame=None):
        '''Yu: A color-based lane detection'''
    ...
    def get_line_markings(self, frame=None):
        ...
        self.lane_line_markings = r_thresh
...
def main():
    ...
    while cap.isOpened():
        ...
        original_frame = cv2.imread(filename)
        ''' 
        cv2.imshow('orig frame',original_frame)
        # print(f"{original_frame.shape}, {type(original_frame)}")
        ...
        # print(f"{lane_line_markings.shape}, {type(lane_line_markings)}")

        # cv2.imshow('after marking', lane_line_markings)
        ...
        if np.any(histogram != 0):
            ...
            # left_fit, right_fit = lane_obj.get_lane_line()
            ...
            print('warning: no lanes in the view')
