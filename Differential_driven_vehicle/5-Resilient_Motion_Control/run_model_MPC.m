...
K_mpc_counterclock = optimal_regulation_controller(A_lat_d,B1_lat_d,e0,Q,R,h);   % MPC gain
disp('MPC gain');
disp(K_mpc_counterclock);

K_lqr_counterclock = -dlqr(A_lat_d,B1_lat_d,Q,R,zeros(size(Q,1),size(R,1)));
disp('lqr gain');
disp(K_lqr_counterclock);

Poles = rand(4,1);
K_place_counterclock = -place(A_lat_d,B1_lat_d,Poles);
disp('pole placement gain')
disp(K_place_counterclock);


disp('clockwise')
K_mpc_clock = optimal_regulation_controller(A_lat_d2,B1_lat_d2,e0,Q,R,h);   % MPC gain
disp('MPC gain');
disp(K_mpc_clock);

K_lqr_clock = -dlqr(A_lat_d2,B1_lat_d2,Q,R,zeros(size(Q,1),size(R,1)));
disp('lqr gain');
disp(K_lqr_clock);

Poles = rand(4,1);
K_place_clock = -place(A_lat_d2,B1_lat_d2,Poles);
disp('pole placement gain')
disp(K_place_clock);
